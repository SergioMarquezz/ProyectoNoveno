<?php
    ...
    function saveAdmin(){
        ...
        global $privilegio, $name ,$paterno, $materno, $calle, $col, $number, $celular, $sexo, $user,
               ...
               if(($name == "") || ($paterno== "") || ($materno == "")){

                    $result['result'] = "datos vacios";
                    print json_encode($result);

               }else if($calle == "" || $col== "" || $number == ""){

                    $result['result'] = "address vacia";
                    print json_encode($result);

               }elseif($celular == "" ){

                    $result['result'] = "cel vacio";
                    print json_encode($result);

               }elseif($sexo == false){

                    $result['result'] = "genero vacio";
                    print json_encode($result);

               }elseif($privilegio == false){

                    $result['result'] = "privelegio vacio";
                    print json_encode($result);

               }else if($user == "" || $email == "" || $pass == "" || $pass_confirm == ""){

                    $result['result'] = "cuenta vacia";
                    print json_encode($result);
               }else{
